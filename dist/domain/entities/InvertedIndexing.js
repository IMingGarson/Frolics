...
exports.InvertedIndexing = void 0;
/**
 ...
 */
function InvertedIndexing(documentObject, totalDocument, totalDocumentLength) {
    /**
    ...
    Object.values(documentObject).forEach((v) => {
        ...
        let docId = v.docId;
        // document length
        let docLength = v.docLength;
        Object.entries(v.terms).forEach((term) => {
            let text = term[0], freq = term[1];
            let node = { docId: docId, freq: freq, docLength: docLength };
            if (!(text in DTM)) {
                DTM[text] = { [docId]: node };
            }
            else {
                DTM[text] = Object.assign(Object.assign({}, DTM[text]), { [docId]: node });
            }
        });
    });
    /**
    ...
    const weightedData = Object.entries(DTM).map((termData) => {
         ...
         */
        const term = termData[0], docs = termData[1], docLength = Object.keys(termData[1]).length;
        const weightedDataObject = {};
        Object.values(docs).forEach((doc) => {
            ...
            let weightedData = { [doc.docId]: score };
            if (!(term in weightedDataObject)) {
                weightedDataObject[term] = weightedData;
            }
            else {
                weightedDataObject[term] = Object.assign(Object.assign({}, weightedDataObject[term]), weightedData);
            }
        });
    ...
    weightedData.forEach((data) => {
        ...
        let score = Object.values(data)[0];
        if (!(term in returnedData)) {
            returnedData[term] = score;
        }
        else {
            returnedData[term] = Object.assign(Object.assign({}, returnedData[term]), score);
        }
    });
    return returnedData;
}
exports.InvertedIndexing = InvertedIndexing;
/**
 ...
 * @returns IDF score
 */
function calculateIDF(totalDocument, invertedDocLength) {
 ...
 * @returns BM25 score
 */
function calculateBM25Score(idf, occurrence, docLength, avgDocumentLength) {
    const k1 = 1.2, b = 0.75;
